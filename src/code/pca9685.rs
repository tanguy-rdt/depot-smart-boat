...
use crate::code::gpio_manager::gpio_manager::GpioManager;
...
impl PCA9685 {
    ...
    pub fn init_pca_addr(&mut self){
        self.gpio.i2c_set_slave_addr(PCA9685_I2C_ADDRESS);
    }

    pub fn init_prescaler(&mut self, frequency: f32){
    ...
    pub fn reset_sleep_mode(&mut self){

        println!("Reset sleep mode");

        let mut mode1 = self.read_mode1();

        // Désactiver le mode sommeil (bit 4 à 0)
        mode1 &= !MODE1_SLEEP;
    ...
    pub fn set_led(&mut self, on: u16, off: u16, channel: i32){
        ...
        buffer[1] = (on >> 8) as u8;
        buffer[2] = off as u8;
        buffer[3] = (off >> 8) as u8;

        self.gpio.i2c_write_byte(on_register, buffer[0]);
        self.gpio.i2c_write_byte(on_register+1, buffer[1]);
        self.gpio.i2c_write_byte(off_register, buffer[2]);
        self.gpio.i2c_write_byte(off_register+1, buffer[3]);  
    }

    pub fn read_mode1(&mut self) -> u8 {
    ...
    pub fn restart_pwm_channels(&mut self) {
    ...
    pub fn set_all_led_mode(&mut self){

        // Write 0x01 in register MODE1 to active All Led mode:
        self.gpio.i2c_write_byte(PCA9685_MODE1, 0x01);
    ...
    pub fn set_all_led(&mut self, on: u16, off: u16){
